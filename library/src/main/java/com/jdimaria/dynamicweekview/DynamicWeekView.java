package com.jdimaria.dynamicweekview;
 ...
 */
public class DynamicWeekView extends View {
    ...
    private int mFullHeight = mHourHeight * HOURS_24;
    private double mStartTime = 0;
    private double mEndTime = HOURS_24;
    private double mVisibleHours = HOURS_24;
    private int mCustomHeight = mFullHeight;
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            ViewCompat.postInvalidateOnAnimation(DynamicWeekView.this);
            return true;
        }
    ...
    public DynamicWeekView(Context context) {
        this(context, null);
    }

    public DynamicWeekView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public DynamicWeekView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.DynamicWeekView, 0, 0);
        try {
            mFirstDayOfWeek = a.getInteger(R.styleable.DynamicWeekView_firstDayOfWeek, mFirstDayOfWeek);
            mHourHeight = a.getDimensionPixelSize(R.styleable.DynamicWeekView_hourHeight, mHourHeight);
            mFullHeight = mHourHeight * HOURS_24;
            mTextSize = a.getDimensionPixelSize(R.styleable.DynamicWeekView_textSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, mTextSize, context.getResources().getDisplayMetrics()));
            mHeaderColumnPadding = a.getDimensionPixelSize(R.styleable.DynamicWeekView_headerColumnPadding, mHeaderColumnPadding);
            mColumnGap = a.getDimensionPixelSize(R.styleable.DynamicWeekView_columnGap, mColumnGap);
            mHeaderColumnTextColor = a.getColor(R.styleable.DynamicWeekView_headerColumnTextColor, mHeaderColumnTextColor);
            mNumberOfVisibleDays = a.getInteger(R.styleable.DynamicWeekView_noOfVisibleDays, mNumberOfVisibleDays);
            mHeaderRowPadding = a.getDimensionPixelSize(R.styleable.DynamicWeekView_headerRowPadding, mHeaderRowPadding);
            mHeaderRowBackgroundColor = a.getColor(R.styleable.DynamicWeekView_headerRowBackgroundColor, mHeaderRowBackgroundColor);
            mDayBackgroundColor = a.getColor(R.styleable.DynamicWeekView_dayBackgroundColor, mDayBackgroundColor);
            mHourSeparatorColor = a.getColor(R.styleable.DynamicWeekView_hourSeparatorColor, mHourSeparatorColor);
            mTodayBackgroundColor = a.getColor(R.styleable.DynamicWeekView_todayBackgroundColor, mTodayBackgroundColor);
            mHourSeparatorHeight = a.getDimensionPixelSize(R.styleable.DynamicWeekView_hourSeparatorHeight, mHourSeparatorHeight);
            mTodayHeaderTextColor = a.getColor(R.styleable.DynamicWeekView_todayHeaderTextColor, mTodayHeaderTextColor);
            mEventTextSize = a.getDimensionPixelSize(R.styleable.DynamicWeekView_eventTextSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, mEventTextSize, context.getResources().getDisplayMetrics()));
            mEventTextColor = a.getColor(R.styleable.DynamicWeekView_eventTextColor, mEventTextColor);
            mEventPadding = a.getDimensionPixelSize(R.styleable.DynamicWeekView_hourSeparatorHeight, mEventPadding);
            mHeaderColumnBackgroundColor = a.getColor(R.styleable.DynamicWeekView_headerColumnBackground, mHeaderColumnBackgroundColor);
            mDayNameLength = a.getInteger(R.styleable.DynamicWeekView_dayNameLength, mDayNameLength);
            mOverlappingEventGap = a.getDimensionPixelSize(R.styleable.DynamicWeekView_overlappingEventGap, mOverlappingEventGap);
            mEventMarginVertical = a.getDimensionPixelSize(R.styleable.DynamicWeekView_eventMarginVertical, mEventMarginVertical);
            mXScrollingSpeed = a.getFloat(R.styleable.DynamicWeekView_xScrollingSpeed, mXScrollingSpeed);
        } finally {
            a.recycle();
        }
    ...
    private void drawTimeColumnAndAxes(Canvas canvas) {
        ...
        int n = (int) mStartTime;
        for (int i = 0; i < mVisibleHours; i++) {
            ...
            String time = getDateTimeInterpreter().interpretTime(n);
            n++;
            if (time == null)
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            int i = 0;
            for (int hourNumber = 0; hourNumber < mVisibleHours; hourNumber++) {
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date)) {
                    ...
                    // Calculate top.
                    float top = Math.round(mHourHeight * HOURS_24 * mEventRects.get(i).top / 1440
                            + mCurrentOrigin.y + mHeaderTextHeight + mHeaderRowPadding * 2
                            + mHeaderMarginBottom + mTimeTextHeight/2 + mEventMarginVertical - mHourHeight * mStartTime);
                    ...
                    bottom = Math.round(mHourHeight * HOURS_24 * bottom / 1440 + mCurrentOrigin.y
                            + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom
                            + mTimeTextHeight/2 - mEventMarginVertical - mHourHeight * mStartTime);
    ...
    public void setTimes(double startTime, double endTime) {
        ...
        mEndTime = endTime;
        setVisibleHours(mEndTime - mStartTime);
    }
    ...
    public void setVisibleHours(double visibleHours) {
        mVisibleHours = visibleHours;
        setCustomHeight((int) Math.round(mHourHeight * visibleHours));
    }
    ...
    public void setTodayToStart() {
        if (mStartDate != null)
            mToday = mStartDate;
    }

    public void setTodayToEnd() {
        if (mEndDate != null)
            mToday = mEndDate;
    }

    public void setStartToToday() {
        if (mToday != null)
            mStartDate = mToday;
    }

    public void setEndToToday() {
        if (mToday != null)
            mEndDate = mToday;
    }
     ...
     * </p>
     * @param length Supported values are {@link DynamicWeekView#LENGTH_SHORT} and
     * {@link DynamicWeekView#LENGTH_LONG}.
     */
    @Deprecated
